...
| `bun run tailor <job_id> [--force]` | Drafts `resume-slant.md` and `cover-letter.md` (plus talking points and a gaps list) into `profile\applications\<job>\`, grounded in the profile, the posting, and the rubric's quoted evidence; marks the job `tailored` (from untracked or `shortlisted` — later real-world statuses are never walked back) | One `claude` call |
